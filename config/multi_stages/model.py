...
class FR_52L(nn.Module):
    ...
    def forward(self, x):
        ...
        filter_size = [32, self.nframes, self.nhb, self.nhb]
        out = []
        features = [x]
        
        for i in range(self.num_stage):
            f = self.feature[i](features[-1])  #b, C, T, H, W
            ...
            o_local_expand = Func.conv3d(o, filter_local_expand, stride=1,
                                     padding = (0, (filter_size[2]-1)//2, (filter_size[3]-1)//2)).permute(0, 3, 4, 1, 2)# B,  H, W, 1, 32*7*5*5
            score = []
            center = o_local_expand[:,:,:,:,3*32*5*5:4*32*5*5]
            for ii in range(self.nframes):
                for iii in range(25):
                    score.append(torch.sum(center * o_local_expand[:,:,:,:,(ii*25 +iii)*32:(ii*25 +iii +1)*32], -1))
            score = self.f_softmax(self.torch.cat(score, 3))#B, H, W, 7*5*5
