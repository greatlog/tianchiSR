...
def train_model(model, optimizer,scheduler, dataloader, summery_writer, device,args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for index, (img, label, neighbor, flow, bicubic) in enumerate(dataloader,0):
            Iter += 1
            img = img.float().to(device)
            label = label.float().to(device)
            neighbor = [j.float().to(device) for j in neighbor]
            flow = [j.float().to(device) for j in flow]
            bicubic = bicubic.float().to(device)
            optimizer.zero_grad()
            
            gen_img = model(img, neighbor, flow) + bicubic
