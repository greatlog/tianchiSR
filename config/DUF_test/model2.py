...
class FR_52L(nn.Module):
    def __init__(self, uf=4, num_stage=4, nframes=7, nhb = 5):
        ...
        for i in range(self.num_stage):
            ...
            out = nn.Sequential(
                ...
                nn.Conv3d(out_c,  self.nframes* self.nhb**2 *1**2, 1, 1)
                )
            self.out.append(out)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stage):
            ...
            o = self.out[i](f)
            ds_o = o.shape
            o = torch.reshape(o, [ds_o[0], self.nframes * self.nhb**2, 1, ds_o[2], ds_o[3], ds_o[4]])
            o = self.f_softmax(o)
            out.append(o)
...
#         bic = Func.interpolate(x[:,:,self.nframes//2], scale_factor = 4, mode='bilinear')
        ...
        for i in range(self.num_stage):
            x_c = []
            tf = Func.interpolate(Fx[i][:,:,0, 0], scale_factor=4, mode='bilinear')
            for c in range(3):
                tx = Func.interpolate(x[:,c], scale_factor=4, mode='bilinear')
                t = self.DynFilter3D(tx.unsqueeze(1), tf.unsqueeze(2), [1, self.nframes, self.nhb, self.nhb])
