...
class Net(nn.Module):
    def __init__(self, num_channels, base_filter, feat, num_stages, n_resblock, nFrames, scale_factor):
        ...
        #Reconstruction
        self.output = ConvBlock(feat, num_channels, 3, 1, 1, activation=None, norm=None)
    ...
    def forward(self, x, neigbor, flow):
        ...
        ####Reconstruction
        out = torch.cat(Ht,0)
        att = self.att(out).reshape(x.shape[0], len(neigbor), -1)
        att = F.softmax(att, dim=1)
        output = torch.sum(att * out.reshape(x.shape[0], len(neigbor), -1), 1).reshape(*h.shape)
