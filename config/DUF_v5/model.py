...
class FR_52L(nn.Module):
    ...
    def forward(self, x):
        ...
        ds_f = f.shape
        f = torch.reshape(f, [ds_f[0], 1*25, 1, ds_f[2], ds_f[3], ds_f[4]])
        f = self.f_softmax(f)
...
#         bic = Func.interpolate(x[:,:,self.nframes//2], scale_factor = 4, mode='bilinear')
        ...
        x_c = []
        for c in range(3):
            t = Func.interpolate(x[:, c, self.nframes//2 : self.nframes//2+1], scale_factor=4, mode='bilinear')
            tfx = Func.interpolate(Fx[:,:,0,0], scale_factor=4, mode='bilinear')
            t = self.DynFilter3D(t, tfx.unsqueeze(2), [1, 5, 5])
            x_c.append(t)
