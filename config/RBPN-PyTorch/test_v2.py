...
def parse_args():
    ...
    parser.add_argument('--model_path', type = str, default = 'weights/RBPN_4x.pth')
...
def construct_model(model_path, device):
    ...
    model = RBPN(num_channels=3, base_filter=256,  feat = 64, num_stages=3, n_resblock=5, nframes,  scale_factor=4)
...
def compute_hr(img, model):
    ...
    for row in start_row:
        for col in start_col:
            patch = torch.as_tensor(img[:,:,:,row:row+64, col:col+128]).float().cuda()
            HR_img[:,:,row*4:row*4+64*4, col*4:col*4 + 128*4] = model(patch)[-1].cpu().detach().numpy() * 255
...
#             cv2.imwrite(osp.join(result_img_dir, img_name), img*255)
        ...
        for i in range(0, len(imglist), 1):
            ngh = []
            for j in range(-(nframes)+1, 1, 1):
                index = abs(i+j) if (i+j)<100 else len(imglist)-1 - (i+j)
                ngh.append(cv2.imread(imglist[index]) / 255)
            center = ngh[-1]
            flow = 
            ...
            flow = [get_flow(input, j) for j in ngh[:-1]]
            bicubic = cv2.imresize(center, fx=4, fy=4, interpolation='bilinear')
            batch = [center, ngh, flow]
